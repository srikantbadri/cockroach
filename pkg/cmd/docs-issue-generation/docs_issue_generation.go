...
func initGetJiraIssueFromGitHubIssue(org, repo string, issue int, token string) func(string, string, int, string) string {
	// TODO: add caching logic here
	var jiraGHIssueCache map[string]string
	key := fmt.Sprintf("%s/%s#%d", org, repo, issue)
	return func(org, repo string, issue int, token string) string {
		if ret, ok := jiraGHIssueCache[key]; ok {
...
func getJiraIssueFromRef(ref, token string) string {
	if jiraIssuePartRE.MatchString(ref) {
		...
		getJiraIssueFromGitHubIssue := initGetJiraIssueFromGitHubIssue(split[0], split[1], issueNumber, token)
		...
		getJiraIssueFromGitHubIssue := initGetJiraIssueFromGitHubIssue("cockroachdb", "cockroach", issueNumber, token)
